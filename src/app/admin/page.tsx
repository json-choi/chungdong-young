...
async function getStats(): Promise<Stats> {
  ...
  const [totalResult, publishedResult, thisWeekResult] = await Promise.all([
    db
      .select({ value: count() })
      .from(announcements)
  ...
  const published = publishedResult[0].value;

  return {
    total,
    published,
    draft: total - published,
    thisWeek: thisWeekResult[0].value,
  };
}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 mt-8">
        <Suspense fallback={<ListSkeleton title="최근 수정" />}>
          <RecentAndUpcomingSection />
        </Suspense>
      </div>
    </div>
  );
...
async function RecentAndUpcomingSection() {
  const { recent, upcoming } = await getRecentAndUpcoming();
      ...
      <section className="card-base overflow-hidden">
        <header className="flex items-center justify-between px-5 py-4 border-b border-church-border-soft">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-heading text-[15px] text-church-text">최근 수정</h2>
          </div>
          <Link
        ...
        {recent.length === 0 ? (
          <EmptyState title="아직 공지사항이 없습니다" hint="첫 공지를 작성해 보세요" />
        ) : (
          <ul>
            {recent.map((item) => (
      ...
      <section className="card-base overflow-hidden">
        <header className="flex items-center justify-between px-5 py-4 border-b border-church-border-soft">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-heading text-[15px] text-church-text">예정된 이벤트</h2>
          </div>
        </header>
        {upcoming.length === 0 ? (
          <EmptyState title="예정된 이벤트가 없습니다" hint="공지에 날짜를 입력하면 여기에 표시됩니다" />
        ) : (
          <ul>
            {upcoming.map((item) => (
              <li key={item.id} className="border-b border-church-border-soft last:border-b-0">
...
function ListSkeleton({ title }: { title: string }) {
  return (
    <>
      {[title, "예정된 이벤트"].map((t) => (
        <section key={t} className="card-base overflow-hidden">
          <header className="flex items-center justify-between px-5 py-4 border-b border-church-border-soft">
            <h2 className="font-heading text-[15px] text-church-text">{t}</h2>
          </header>
          <ul>
